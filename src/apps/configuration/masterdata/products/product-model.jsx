...
import {
  ...
  getProductModels,
  removeProductModel,
} from "../../../../data/configuration/masterdata/products/DataConfigProductModel";
...
export default function ProductModel() {
  ...
  const { product_model, loading } = useSelector(
    (state) => state.configProductModel
  );
  const { selectedProductModel } = useSelector(
    (state) => state.configProductModel
  );
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      modl_prod: data.modl_prod,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addProductModel(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getProductModels()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getProductModels()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeProductModel(indx));
      if (removeProductModel.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getProductModelById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      reset({
        modl_prod: selectedProductModel.modl_prod || "",
        desc_shrt: selectedProductModel.desc_shrt || "",
      });
    }
  }, [selectedProductModel, reset, modalMode]);
    ...
    <div className=" w-full bg-white rounded-[4px] mt-[20px] height-custom-fixed shadow-custom">
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: "bold" }}> {product_model.length} </span>{" "}
          of{" "}
          <span style={{ fontWeight: "bold" }}> {product_model.length} </span>{" "}
          results
        </h6>
      ...
      <div className="custom-table">
        <div className="scrollbar-custom">
          <Table responsive className="mb-0">
            ...
            <tbody>
              {product_model.map((item, i) => (
                <tr key={i}>
                  <td>{item.modl_prod}</td>
                  <td>{item.desc_shrt}</td>
                  <td
                    className={`sticky right-[-2px] bg-white z-${
                      product_model.length - (i + 1)
                    }`}
                  >
                    <center>
                      <ActionTableModal
                  ...
                  <Col>
                    <div className=" w-full border-b border-[#e2e5ec] py-[20px]">
                      <InputText
                        ...
                        type="text"
                        disabled={false}
                        placeholder="Please Input Product Model"
                        required={true}
