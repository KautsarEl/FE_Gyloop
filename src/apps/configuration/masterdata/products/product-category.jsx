...
import { addProductCategory, editProductCategory, getProductCategoryById, getProductCategorys, removeProductCategory } from "../../../../data/configuration/masterdata/products/DataConfigProductCategory";
...
export default function ProductCategory() {
  ...
  const { product_category, loading } = useSelector(
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      catg_prod: data.catg_prod,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addProductCategory(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getProductCategorys()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getProductCategorys()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeProductCategory(indx));
      if (removeProductCategory.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getProductCategoryById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      reset({
        catg_prod: selectedProductCategory.catg_prod || "",
        desc_shrt: selectedProductCategory.desc_shrt || "",
      });
    }
  }, [selectedProductCategory, reset, modalMode]);
