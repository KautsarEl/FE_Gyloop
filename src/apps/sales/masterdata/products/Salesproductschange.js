...
export default function Salesproductschange() {
  const { id } = useParams();
  ...
  useEffect(() => {
  if (id) {
    axios.get(`http://203.175.11.244:8080/api/products/${id}`)
      .then((res) => {
        methods.reset(res.data); // ✅ Reset seluruh form
      })
      .catch((err) => {
        ...
        <div className="main main-table p-3 p-lg-4 mb-[60px]">
          <div className="d-md-flex align-items-center justify-content-between mb-4">
            <div>
              ...
              <h4 className="main-title mb-0">Edit Product</h4>
            </div>
            <div className="d-flex gap-2 mt-3 mt-md-0">
              <Link
                to={`/apps/sales/masterdata/products/salesproductsdetail/${id}`}
              >
                <Button
                  variant=""
                  className="btn-white d-flex align-items-center gap-2"
                >
                  <i className={` fars-glasses`} style={{ fontSize: 14 }}></i>
                  Display
                </Button>
              </Link>
              <Button
