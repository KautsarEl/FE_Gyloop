...
import {
  addSpecialIndicator,
  editSpecialIndicator,
  getSpecialIndicatorById,
  getSpecialIndicators,
  removeSpecialIndicator,
} from "../../../../data/configuration/masterdata/products/DataConfigSpecialIndicator";

export default function SpecialIndicator() {
  ...
  const { special_indicator, loading } = useSelector(
    (state) => state.configSpecialIndicator
  );
  const { selectedSpecialIndicator } = useSelector(
    (state) => state.configSpecialIndicator
  );
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      indc_spcl: data.indc_spcl,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addSpecialIndicator(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getSpecialIndicators()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getSpecialIndicators()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeSpecialIndicator(indx));
      if (removeSpecialIndicator.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getSpecialIndicatorById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      selectedSpecialIndicator
    ) {
      reset({
        indc_spcl: selectedSpecialIndicator.indc_spcl || "",
        desc_shrt: selectedSpecialIndicator.desc_shrt || "",
      });
    }
  }, [selectedSpecialIndicator, reset, modalMode]);
    ...
    <div className=" w-full bg-white rounded-[4px] mt-[20px] height-custom-fixed shadow-custom">
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: "bold" }}> {special_indicator.length} </span>{" "}
          of{" "}
          <span style={{ fontWeight: "bold" }}> {special_indicator.length} </span>{" "}
          results
        </h6>
      ...
      <div className="custom-table">
        <div className="scrollbar-custom">
          <Table responsive className="mb-0">
            <thead>
              <tr>
                <th scope="col">Special Indicator</th>
            ...
            <tbody>
              {special_indicator.map((item, i) => (
                <tr key={i}>
                  <td>{item.indc_spcl}</td>
                  <td>{item.desc_shrt}</td>
                  <td
                    className={`sticky right-[-2px] bg-white z-${
                      special_indicator.length - (i + 1)
                    }`}
                  >
                    <center>
                      <ActionTableModal
        ...
        <ModalConfig
          ...
          title={"Create Special Indicator"}
          BodyModal={
        ...
        <ModalConfig
          ...
          title={"Display Special Indicator"}
          BodyModal={
            <FormProvider {...methods}>
              <Form onSubmit={methods.handleSubmit(handleSubmit)}>
                <Row className=" flex flex-col gap-[25px] mb-[25px] px-[30px]">
        ...
        <ModalConfig
          ...
          title={"Change Special Indicator"}
          BodyModal={
            <FormProvider {...methods}>
