...
export default function Licenses() {
  ...
  const columns = licenses_table.map((column) => column.name_fild); // Use 'name' for column headers
  const data = userLicensesHeader.map(
    (row) => licenses_table.map((column) => row[column.vlue_fild]) // Map data to match columns
  );
...
//   );
  ...
  const [filteredData, setFilteredData] = useState(userLicensesHeader);

  const handleSearch = (e) => {
    const value = e.target.value.toLowerCase();
    setSearch(value);

    const filtered = userLicensesHeader.filter((item) =>
        ...
        <div className="d-md-flex align-items-center justify-content-between mb-4">
          <div>
            ...
            <h4 className="main-title mb-0">
              {show === "Edit" ? "Edit" : "Display"} Licenses
              Configuration
            </h4>
          </div>
          ...
          <div className="custom-table" style={{ position: "relative" }}>
            ...
            >
              ...
              <div className=" container-action-table">
                <Link to={'/apps/administration/licensesdetail'}>
                  <Button
                    variant="primary"
