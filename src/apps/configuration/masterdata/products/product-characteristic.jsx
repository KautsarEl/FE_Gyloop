...
import { addProductCharacteristic, editProductCharacteristic, getProductCharacteristicById, getProductCharacteristics, removeProductCharacteristic } from "../../../../data/configuration/masterdata/products/DataConfigProductCharacteristic";
...
export default function ProductCharacteristic() {
  ...
  const { product_characteristic, loading } = useSelector(
    (state) => state.configProductCharacteristic
  );
  const { selectedProductCharacteristic } = useSelector(
    (state) => state.configProductCharacteristic
  );
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      char_prod: data.char_prod,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addProductCharacteristic(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getProductCharacteristics()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getProductCharacteristics()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeProductCharacteristic(indx));
      if (removeProductCharacteristic.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getProductCharacteristicById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      selectedProductCharacteristic
    ) {
      reset({
        char_prod: selectedProductCharacteristic.char_prod || "",
        desc_shrt: selectedProductCharacteristic.desc_shrt || "",
      });
    }
  }, [selectedProductCharacteristic, reset, modalMode]);
      ...
      <div className="custom-table">
        <div className="scrollbar-custom">
          <Table responsive className="mb-0">
            <thead>
              <tr>
                <th scope="col">Product Characteristic</th>
            ...
            <tbody>
              {product_characteristic.map((item, i) => (
                <tr key={i}>
                  <td>{item.char_prod}</td>
                  <td>{item.desc_shrt}</td>
                  <td
                    className={`sticky right-[-2px] bg-white z-${
                      product_characteristic.length - (i + 1)
                    }`}
                  >
                    <center>
                      <ActionTableModal
                  ...
                  <Col>
                    <div className=" w-full border-b border-[#e2e5ec] py-[20px]">
                      <InputText
                        ...
                        type="text"
                        disabled={false}
                        placeholder="Please Input Product Characteristic"
                        required={true}
        ...
        <ModalConfig
          ...
          title={"Display Product Characteristic"}
          BodyModal={
            <FormProvider {...methods}>
              <Form onSubmit={methods.handleSubmit(handleSubmit)}>
                <Row className=" flex flex-col gap-[25px] mb-[25px] px-[30px]">
        ...
        <ModalConfig
          ...
          title={"Change Product Characteristic"}
          BodyModal={
            <FormProvider {...methods}>
