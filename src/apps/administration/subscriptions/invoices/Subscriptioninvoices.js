...
import { subscriptionsInvoiceHeader } from "../../../../data/administration/subscriptions/invoices/DataSubscriptionInvoices";
import { subscriptionsInvoiceItem } from "../../../../data/administration/subscriptions/invoices/DataSubscriptionInvoices";
import { subscriptioninvoiceheader_table } from "../../../../data/administration/subscriptions/invoices/DataSubscriptionInvoicesTable";
import { subscriptioninvoiceitem_table } from "../../../../data/administration/subscriptions/invoices/DataSubscriptionInvoicesTable";
...
export default function Subscriptioninvoices() {
  ...
  const columns = subscriptioninvoiceheader_table.map((column) => column.name_fild); // Use 'name' for column headers
  const data = subscriptionsInvoiceHeader.map(
    (row) => subscriptioninvoiceheader_table.map((column) => row[column.vlue_fild]) // Map data to match columns
  );
...
//   );
  ...
  const [filteredData, setFilteredData] = useState(subscriptionsInvoiceHeader);

  const handleSearch = (e) => {
    const value = e.target.value.toLowerCase();
    setSearch(value);

    const filtered = subscriptionsInvoiceHeader.filter((item) =>
        ...
        <div className="d-md-flex align-items-center justify-content-between mb-4">
          <div>
            ...
            <h4 className="main-title mb-0">
              {show === "Edit" ? "Edit" : "Display"} Invoices
            ...
            >
              ...
              >
                ...
                <span style={{ fontWeight: "bold" }}>
                  {" "}
                  {subscriptionsInvoiceHeader.length}{" "}
                </span>{" "}
                of{" "}
                <span style={{ fontWeight: "bold" }}>
                  {" "}
              ...
              <div className=" container-action-table">
                <Link to={'/apps/administration/subscriptions/subscriptioninvoicesdetail'}>
                  <Button
                    variant="primary"
                    className="d-flex align-items-center gap-2"
                  >
                    <i className="fars-glasses"></i>
