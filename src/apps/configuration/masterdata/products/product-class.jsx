...
import {
  ...
  getProductClasss,
  removeProductClass,
} from "../../../../data/configuration/masterdata/products/DataConfigProductClass";
...
export default function ProductClass() {
  ...
  const { product_class, loading } = useSelector(
    (state) => state.configProductClass
  );
  const { selectedProductClass } = useSelector(
    (state) => state.configProductClass
  );
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      clas_prod: data.clas_prod,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addProductClass(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getProductClasss()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getProductClasss()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeProductClass(indx));
      if (removeProductClass.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getProductClassById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      reset({
        clas_prod: selectedProductClass.clas_prod || "",
        desc_shrt: selectedProductClass.desc_shrt || "",
      });
    }
  }, [selectedProductClass, reset, modalMode]);
    ...
    <div className=" w-full bg-white rounded-[4px] mt-[20px] height-custom-fixed shadow-custom">
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: "bold" }}> {product_class.length} </span>{" "}
          of{" "}
          <span style={{ fontWeight: "bold" }}> {product_class.length} </span>{" "}
          results
        </h6>
                  ...
                  <Col>
                    <div className=" w-full border-b border-[#e2e5ec] py-[20px]">
                      <InputText
                        ...
                        type="text"
                        disabled={false}
                        placeholder="Please Input Product Class"
                        required={true}
