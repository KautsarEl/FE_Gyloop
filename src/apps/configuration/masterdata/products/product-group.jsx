...
import {
  ...
  getProductGroups,
  removeProductGroup,
} from "../../../../data/configuration/masterdata/products/DataConfigProductGroup";
...
export default function ProductGroup() {
  ...
  const { product_group, loading } = useSelector(
    (state) => state.configProductGroup
  );
  const { selectedProductGroup } = useSelector(
    (state) => state.configProductGroup
  );
  ...
  const handleSubmit = async (data) => {
    const form = {
      idno_systm: "SYS001",
      idno_tent: "TENANT001",
      grop_prod: data.grop_prod,
      desc_shrt: data.desc_shrt,
      crdt_by: "admin",
      updt_by: "admin",
    };
    await dispatch(addProductGroup(form));
    alert("Data successfully added");
    handleClose(); // Close modal after submit
    dispatch(getProductGroups()); // Refresh data setelah tambah
  };
  const handleUpdated = async (data) => {
    ...
    dispatch(getProductGroups()); // Refresh data
  };
  const handleDelete = async (indx) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      const result = await dispatch(removeProductGroup(indx));
      if (removeProductGroup.fulfilled.match(result)) {
  ...
  const handleShowChange = async (indx) => {
    setModalMode("change");
    await dispatch(getProductGroupById(indx));
    setSelectedId(indx);
    setShowChange(true);
  };
  ...
  useEffect(() => {
    if (
      ...
      reset({
        grop_prod: selectedProductGroup.grop_prod || "",
        desc_shrt: selectedProductGroup.desc_shrt || "",
      });
    }
  }, [selectedProductGroup, reset, modalMode]);
    ...
    <div className=" w-full bg-white rounded-[4px] mt-[20px] height-custom-fixed shadow-custom">
      ...
      >
        ...
        >
          ...
          <span style={{ fontWeight: "bold" }}> {product_group.length} </span>{" "}
          of{" "}
          <span style={{ fontWeight: "bold" }}> {product_group.length} </span>{" "}
          results
        </h6>
      ...
      <div className="custom-table">
        <div className="scrollbar-custom">
          <Table responsive className="mb-0">
            ...
            <tbody>
              {product_group.map((item, i) => (
                <tr key={i}>
                  <td>{item.grop_prod}</td>
                  <td>{item.desc_shrt}</td>
                  <td
                    className={`sticky right-[-2px] bg-white z-${
                      product_group.length - (i + 1)
                    }`}
                  >
                    <center>
                      <ActionTableModal
                  ...
                  <Col>
                    <div className=" w-full border-b border-[#e2e5ec] py-[20px]">
                      <InputText
                        ...
                        type="text"
                        disabled={false}
                        placeholder="Please Input Product Group"
                        required={true}
